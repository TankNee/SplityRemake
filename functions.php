...
{ //获取两个参数，文章的ID和需要显示的图片下标
    // 当文章无图片时的默认缩略图
    $rand = rand(1, 3);
    $random = $widget->widget('Widget_Options')->themeUrl . '/images/thumbs/random/' . $rand . '.jpg'; // 随机缩略图路径
...
function PreprocessTextContent($article)
{
    ...
    $img_replacement = '<a href="$1" data-fancybox="gallery" /><img src="$1" alt="' . $article->title . '" title="点击放大图片"></a>';
    $content = preg_replace($img_pattern, $img_replacement, $article->content);
    // 按钮样式解析--这么做是为了兼容handsome格式文章的迁移！
    $btn_pattern = '/\[button.*?color\=\"(.*?)\".*?icon\=\"(.*?)\".*?url\=\"(.*?)\".*?type\=\"(.*?)\"[^>]*](.*)\[\/button\]/i';
    if (preg_match_all($btn_pattern, $content, $button)) {
    }
    echo $content;
}
function PreprocessComment($comments)
{
    // $pattern = '/\[secret]([\s\S]*)\[\/secret\]/i';
    // $private = '<p>****私密评论****</p>';
    // if (strstr($comments->content(), '[secret]')) {
    //     // echo $private;
    // } else {
